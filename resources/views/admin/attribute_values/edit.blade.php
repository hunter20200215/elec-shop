...
@section('content')
    <div class="content-wrapper">
        <section class="content-header">
            <div class="content-header-text">@lang('admin.Attribute values') / @lang('admin.edit')</div>
        </section>
        <section class="content-container">
            <form method="POST" class="crud-form">
                <input type="hidden" name="color" id="color" value="{{ $attribute_value->color }}">
                @csrf
                <div class="form-group col-md-6" >
                    <label for="value">@lang('admin.Value')</label>
                    <input value="{{ $attribute_value->value }}" type="text" class="form-control @error('value') input-error-danger @enderror" id="value" name="value" placeholder="@lang('admin.Value')">
...
@section('custom_scripts')
    ...
    <script>
        ...
        const colorPicker = Pickr.create({
            ...
            default: '{{ $attribute_value->color ?? 'transparent'}}',
            components: {
                preview: true,
                opacity: true,
                hue: true,
